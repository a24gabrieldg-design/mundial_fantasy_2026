...
function matchCardHTML(m, preds){
  // Permitir reabrir/cerrar predicciones manualmente (admin total)
  ...
  // Admin total puede forzar manualmente el estado de cierre/reapertura.
  // Guardamos flags en localStorage para no tocar aún Firestore.
  const forcedLockKey = `wf26_forced_lock_${m.id}`;
  const forcedLockRaw = localStorage.getItem(forcedLockKey);
  // forcedLockRaw: null (sin override), '1' (cerrado), '0' (reabierto)
  ...
  // Estado visual cuando se cierra la predicción por tiempo/admin, pero aún NO hay resultado.
  // Pides: (1) marcador principal en 0-0, (2) un pequeño al lado con 0-0, (3) texto/estilo en verde.
  // Cuando termine: se vuelve a color/estilo normal y al lado aparecerán los puntos.
  const showPredictionClosedUi = locked && !finished;
  const predSmall = showPredictionClosedUi
    ? `<span class="pred-small">0-0</span>`
    : (finished
      ? `<span class="pred-small">+${calcPoints(res, p)} pts</span>`
      : '');
...
if(isTotalAdmin()){
  ...
  // Controles admin para abrir/cerrar manualmente las predicciones del partido
  ...
  return `<div class="match-card${finished?' finished':''}" id="mc-${m.id}">
    ...
    <div class="match-row">
      <div class="team-side">
        ...
        <input class="score-input" type="number" min="0" max="20" value="${predForUiClosed?predForInputs.g1:p.g1}" id="pred-${m.id}-1" ${locked?'disabled':''} oninput="savePred('${m.id}')">
      </div>
...
function forceToggleLock(mid, val){
  // val: '0' (reabierto), '1' (cerrado), 'clear'
...
async function adminRecalc(mid){
  // Recalcula puntos según calcPoints() y vuelve a renderizar.
...
function calcPoints(res, pred){
  ...
  // Si no hay predicción completa, no puntúa.
  ...
  // Goles acertados para un equipo (g1 o g2)
  ...
  // Reglas pedidas:
  // - Acertar los goles de un equipo (y NO es exacto): 1 punto
  if(acertaAlgunGol && !acertaAmbosGoles && !acertaGanador) return 1;

  // - Acertar el ganador pero no aciertas la cantidad de goles de ningun equipo: 2 puntos
  if(acertaGanador && !acertaAlgunGol) return 2;

  // - Acertar el ganador y los goles de un solo equipo (no exacto): 4 puntos
  //   (Esto incluye el caso de acertar un gol en el lado correcto aunque no sea exacto)
  if(acertaGanador && acertaAlgunGol && !acertaAmbosGoles) return 4;
