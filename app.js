...
async function doLogin(){
  try{
    ...
    showMain();
    // Cargar cruces KO en background, sin bloquear el login
    loadKOMatchesFromFirestore().catch(()=>{});
  }catch(e){
...
function renderKOMatches(){
  ...
  const preds=((S.predictions[S.currentLeague]||{})[S.currentUser])||{};
  const teamsOpts = ALL_TEAMS.map(t=>`<option value="${t.n}">${t.f} ${t.n}</option>`).join('');
  const cards = matches.map(m=>{
    let html = matchCardHTML(m,preds);
    if(!isTotalAdmin()) return html;
    const editorRow = `<div class="admin-row" style="flex-wrap:wrap;gap:6px;align-items:center">
      <span style="font-size:10px;color:var(--admin);font-weight:700">CRUCE:</span>
      <select id="ko-t1-${m.id}" class="admin-input" style="width:130px;padding:4px 4px">
        <option value="">-- Equipo 1 --</option>${teamsOpts}
      </select>
      <span style="font-size:11px;color:var(--text3)">vs</span>
      <select id="ko-t2-${m.id}" class="admin-input" style="width:130px;padding:4px 4px">
        <option value="">-- Equipo 2 --</option>${teamsOpts}
      </select>
      <button class="btn-admin" onclick="adminSetKOTeams('${m.id}',${S.currentPhase})">✅ Definir</button>
    </div>`;
    // Insertar el editorRow justo antes del cierre del match-card
    return html.slice(0, html.lastIndexOf('</div>')) + editorRow + '</div>';
  }).join('');
  c.innerHTML = cards + `<div class="save-indicator" id="save-ind"></div>`;
}
...
async function adminSetKOTeams(mid, phase){
  const sel1 = document.getElementById('ko-t1-'+mid);
  const sel2 = document.getElementById('ko-t2-'+mid);
  if(!sel1||!sel2) return;
  const n1 = sel1.value.trim();
  const n2 = sel2.value.trim();
  if(!n1||!n2){ alert('Selecciona los dos equipos'); return; }
  if(n1===n2){ alert('Los dos equipos deben ser diferentes'); return; }
  const flag1 = ALL_TEAMS.find(t=>t.n===n1)?.f||'❓';
  const flag2 = ALL_TEAMS.find(t=>t.n===n2)?.f||'❓';
  const template = (S.knockoutMatches[phase]||KNOCKOUT_TEMPLATES[phase]).map(m=>({...m}));
  const idx = template.findIndex(m=>m.id===mid);
  if(idx===-1){ alert('Partido no encontrado'); return; }
  template[idx] = { ...template[idx], n1, t1:flag1, n2, t2:flag2, locked:false };
  S.knockoutMatches[phase] = template;
  try{
    const { doc, setDoc } = await import('https://www.gstatic.com/firebasejs/11.6.0/firebase-firestore.js');
    await setDoc(doc(fbDb(),'koMatches',String(phase)), { matches:template, updatedAt:Date.now() }, { merge:false });
    renderPhaseBody();
    const ind=document.getElementById('save-ind');
    if(ind){ ind.textContent=`✅ ${flag1} ${n1} vs ${flag2} ${n2}`; setTimeout(()=>{ if(ind) ind.textContent=''; },2500); }
  }catch(e){
    console.error('adminSetKOTeams error', e);
    alert('Error al guardar cruce: '+(e?.message||String(e)));
  }
}
...
if(S.currentUser){
  (async ()=>{
    ...
    showMain();
    // Cargar cruces KO en background
    loadKOMatchesFromFirestore().catch(()=>{});
  })();
}
